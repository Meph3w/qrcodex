...
export default async function BlogPostPagePT({ params }: { params: { slug: string } }) {
  const post = await getPostBySlug(params.slug, "pt")
    ...
    <div className="container mx-auto py-8 px-4">
      <div className="max-w-3xl mx-auto">
        <Button asChild variant="ghost" className="mb-6">
          <Link href="/pt/blog">
            <ChevronLeft className="mr-2 h-4 w-4" /> Voltar para o Blog
          </Link>
        </Button>
        ...
        <article>
          ...
          <p className="text-gray-500 mb-8">
            {new Date(post.date as string).toLocaleDateString("pt-BR", {
              year: "numeric",
              month: "long",
              day: "numeric",
            })}
          ...
          <div className="mt-12 pt-6 border-t border-gray-200">
            <Link href="/pt/#qr-generator">
              <Button size="lg" className="w-full py-6 text-lg">
                <QrCode className="mr-2 h-5 w-5" />
                Crie seu Código QR gratuitamente agora
              </Button>
            </Link>
          </div>
        </article>
